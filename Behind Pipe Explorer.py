...
with st.sidebar:
    ...
    vsh_cutoff = st.slider("VSH Cutoff (%)", 0, 100, 50, key="vsh_slider")
    sw_cutoff = st.slider("Sw Cutoff (%)", 0, 100, 50, key="sw_slider")
    phit_cutoff = st.slider("Porosity Cutoff (%)", 0, 30, 10, key="phit_slider")
...
def process_tops(tops_file) -> None:
    try:
        ...
        if not all(col in tops_df.columns for col in expected_columns):
            tops_df.columns = expected_columns  # Force column names if mismatch
...
if st.session_state.well_data:
    ...
    # Calculate net reservoir and net pay
    if all(col in df.columns for col in ['VSH', 'SW', 'PHIT']):
        df['NET_RESERVOIR'] = (df['VSH'] <= vsh_cutoff / 100).astype(int)
        df['NET_PAY'] = ((df['NET_RESERVOIR'] == 1) & (df['SW'] <= sw_cutoff / 100) &
                         (df['PHIT'] >= phit_cutoff / 100)).astype(int)
    else:
        df['NET_RESERVOIR'] = 0
        df['NET_PAY'] = 0
    ...
    if 'NET_RESERVOIR' in df.columns and st.session_state.show_net_reservoir:
        available_tracks.append('net_reservoir')
    if 'NET_PAY' in df.columns and st.session_state.show_net_pay:
    ...
    if not selected_tracks:
        ...
        for i, track in enumerate(selected_tracks):
            ...
            if track == 'tops':
                ...
                ax.axvline(phit_cutoff, color='red', ls=':', lw=1)
                ax.legend(fontsize=8)
                ax.set_xlim(0, df['PHIT'].max() * 100 * 1.1)

            elif track == 'sw':
                ax.set_title("Water Saturation (%)", fontsize=10)
                ax.plot(df['SW'] * 100, df['DEPTH'], color=colors['saturation'], lw=1.5)
                ax.axvline(sw_cutoff, color='red', ls=':', lw=1)
                ax.set_xlim(0, 100)
        ...
        with tab1:
            ...
            summary_df = df[[col for col in columns if col in df.columns]].round(3)
            ...
            if unperf_df.empty:
                ...
                if 'tops' in well:
                    ...
                    for i, row in grouped.iterrows():
                        # Find the top that is just above or at the interval's top depth
                ...
                grouped['Avg_Porosity'] = (grouped['Avg_Porosity'] * 100).round(2)  # Convert to percentage
                grouped['Avg_Sw'] = (grouped['Avg_Sw'] * 100).round(2)  # Convert to percentage
